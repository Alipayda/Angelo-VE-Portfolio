...
const Store = (() => {
    ...
    if (sbClient && key !== 'session' && key !== 'adminSession') {
      try {
        if (key === 'users') {
          ...
          for (const p of val) { await sbClient.from('posts').upsert(p); }
  ...
  if (sbClient) {
    ...
    ['posts', 'comments', 'announcements'].forEach(cat => {
    ...
    sbClient.channel('public:dynamic')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'posts' }, payload => {
          sbClient.from('posts').select('*').then(({ data }) => {
              if (data) _localSet('posts', data); window.dispatchEvent(new CustomEvent('ps_db_updated', { detail: 'posts' }));
          });
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'comments' }, payload => {
          sbClient.from('comments').select('*').then(({ data }) => {
              if (data) _localSet('comments', data); window.dispatchEvent(new CustomEvent('ps_db_updated', { detail: 'comments' }));
          });
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'announcements' }, payload => {
          sbClient.from('announcements').select('*').then(({ data }) => {
              if (data) _localSet('announcements', data); window.dispatchEvent(new CustomEvent('ps_db_updated', { detail: 'announcements' }));
          });
      }).subscribe();
